...
static void usage(const char *prog) {
    fprintf(stderr,
            ...
            "\n"
            "Environment tunables (defaults in parentheses):\n"
            "  DIRECT_COPY_MAX_WORKERS (256)         total worker threads\n"
            "  DIRECT_COPY_SMALL_MAX_WORKERS (32)    concurrent small-file copies\n"
            "  DIRECT_COPY_LARGE_WORKERS (6)         concurrent large files\n"
            "  DIRECT_COPY_LARGE_READERS (4) / DIRECT_COPY_LARGE_WRITERS (2)\n"
            "                                        per-large-file pipeline split\n"
            "  DIRECT_COPY_LARGE_FILE_INFLIGHT (16)  chunk buffers per large file\n"
            "  DIRECT_COPY_CHUNK_MB (1)              copy chunk size\n"
            "  DIRECT_COPY_LARGE_THRESHOLD_MB (10)   small/large classification boundary\n"
            "  DIRECT_COPY_DIRECT_IO_MIN_SIZE_MB (10) direct I/O only at/above this size\n"
            "                                        (0 = direct for all sizes)\n"
            "  DIRECT_COPY_SMALL_WRITE_DIRECT_MIN_KB (64)\n"
            "                                        small files at/above this size write\n"
            "                                        their aligned bulk O_DIRECT (0 = never)\n"
            "  DIRECT_COPY_DISABLE_DIRECT_IO (=1)    force buffered I/O everywhere;\n"
            "                                        ..._READ_/..._WRITE_ variants split it\n"
            "  DIRECT_COPY_DISABLE_COPY_FILE_RANGE (=1) never use copy_file_range(2)\n"
            "  DIRECT_COPY_TRAVERSAL_WORKERS (8)     parallel directory scanners\n"
            "  DIRECT_COPY_MAX_QUEUED_FILES (262144) dispatch queue bound\n"
            "  DIRECT_COPY_VERIFY_QUEUE_MAX (262144) verify intake queue bound\n"
            "  DIRECT_COPY_VERIFY_WORKERS            same as --verify-workers\n"
            "  DIRECT_COPY_SSH_CONNECTIONS (4)       ssh:// transport connections\n",
            prog);
}
